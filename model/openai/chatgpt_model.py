...
g_model_openai_config = model_conf(const.OPEN_AI)
api_base = g_model_openai_config.get('api_base')
api_key = g_model_openai_config.get('api_key')
proxy = g_model_openai_config.get('proxy')
...
log.info("[CHATGPT] init OpenAI, api_base={:s} proxy={:s}".format(api_base, proxy))
...
class ChatGPTModel(Model):
    def __init__(self):
        ...
        # if proxy:            
            ...
            # openai.proxy = proxy
        log.info("[CHATGPT] api_base={} proxy={}".format(api_base, proxy))
    ...
    async def reply_text_stream(self, query,  context, retry_count=0):
        try:
            ...
            res = client.chat.completions.create(model= model_conf(const.OPEN_AI).get("model") or "gpt-3.5-turbo",  # 对话模型的名称
            messages=new_query,
            temperature=model_conf(const.OPEN_AI).get("temperature", 0.75),  # 熵值，在[0,1]之间，越大表示选取的候选词越随机，回复越具有不确定性，建议和top_p参数二选一使用，创意性任务越大越好，精确性任务越小越好
            #max_tokens=4096,  # 回复最大的字符数，为输入和输出的总数
            #top_p=model_conf(const.OPEN_AI).get("top_p", 0.7),,  #候选词列表。0.7 意味着只考虑前70%候选词的标记，建议和temperature参数二选一使用
